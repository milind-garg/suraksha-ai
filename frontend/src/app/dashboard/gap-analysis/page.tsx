...
import {
  ...
  TrendingUp, Brain, ArrowRight, IndianRupee,
  Heart, Car, Home, Plane, Users, Star,
...
export default function GapAnalysisPage() {
  ...
  const runGapAnalysis = async () => {
    ...
    const gaps: GapItem[] = RECOMMENDED_COVERAGE.map(rec => {
      ...
      const matchingPolicies = policies.filter(p => p.policyType === rec.type)
      const currentCoverage = matchingPolicies.reduce((sum, p) => sum + (p.sumInsured || 0), 0)
      ...
      const gapPercent = Math.min(100, Math.round((currentCoverage / rec.idealCoverage) * 100))
    ...
    const totalCurrentCoverage = gaps.reduce((sum, g) => sum + g.currentCoverage, 0)
    const totalRecommendedCoverage = gaps.reduce((sum, g) => sum + g.recommendedCoverage, 0)

    const topRecommendations = []
    const topRecommendationsHindi = []

    if (gaps.find(g => g.type === 'health' && g.status !== 'good')) {
      topRecommendations.push('Get health insurance immediately — medical costs can wipe out savings')
      topRecommendationsHindi.push('तुरंत स्वास्थ्य बीमा लें — चिकित्सा खर्च बचत खत्म कर सकता है')
    }
    if (gaps.find(g => g.type === 'life' && g.status !== 'good')) {
      topRecommendations.push('Buy term life insurance — cheapest way to protect your family')
      topRecommendationsHindi.push('टर्म लाइफ इंश्योरेंस खरीदें — परिवार की सुरक्षा का सबसे सस्ता तरीका')
    }
    if (gaps.find(g => g.type === 'health' && g.status === 'low')) {
      topRecommendations.push('Increase health cover — add a super top-up plan for low premium')
      topRecommendationsHindi.push('स्वास्थ्य कवर बढ़ाएं — कम प्रीमियम पर सुपर टॉप-अप प्लान लें')
    }
    topRecommendations.push('Review all policies annually at renewal for better terms')
    topRecommendationsHindi.push('बेहतर शर्तों के लिए नवीनीकरण पर सभी पॉलिसियों की सालाना समीक्षा करें')
  ...
  const getScoreLabel = (score: number) => {
    ...
    if (score >= 60) return { en: 'Moderately Protected', hi: 'मध्यम सुरक्षित' }
    if (score >= 40) return { en: 'Partially Protected', hi: 'आंशिक रूप से सुरक्षित' }
        ...
        {result && !isAnalyzing && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Card className="border-0 shadow-sm md:col-span-1">
                <CardContent className="p-6 text-center">
                  <p className="text-sm text-gray-500 mb-2">Overall Protection Score</p>
                  <p className="text-xs text-blue-500 hindi-text mb-4">समग्र सुरक्षा स्कोर</p>
                  ...
                  <div className={`text-sm hindi-text ${getScoreColor(result.overallScore)}`}>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-green-50 rounded-xl p-4">
                      <p className="text-xs text-gray-500">Your Current Coverage</p>
                      <p className="text-xs text-green-600 hindi-text">आपका मौजूदा कवरेज</p>
                      <p className="text-2xl font-bold text-green-700 mt-1">
                        ₹{(result.totalCurrentCoverage / 100000).toFixed(1)}L
                      </p>
                    </div>
                    <div className="bg-blue-50 rounded-xl p-4">
                      <p className="text-xs text-gray-500">Recommended Coverage</p>
                      <p className="text-xs text-blue-600 hindi-text">अनुशंसित कवरेज</p>
                  ...
                  <div className="flex gap-3">
                    {[
                      { label: 'Well Covered', hindi: 'अच्छा', count: result.gaps.filter(g => g.status === 'good').length, color: 'bg-green-100 text-green-700' },
                      { label: 'Low Coverage', hindi: 'कम', count: result.gaps.filter(g => g.status === 'low').length, color: 'bg-yellow-100 text-yellow-700' },
                      { label: 'Not Covered', hindi: 'नहीं', count: result.gaps.filter(g => g.status === 'missing').length, color: 'bg-red-100 text-red-700' },
                    ].map((item, i) => (
                      <div key={i} className={`flex-1 rounded-xl p-3 text-center ${item.color}`}>
              ...
              <Card className="border-0 shadow-sm border-l-4 border-l-red-500">
                <CardHeader className="pb-2">
                  <CardTitle className="text-base text-red-700 flex items-center gap-2">
                    ...
                    Critical Gaps — Action Required / गंभीर कमियां — तुरंत कार्रवाई जरूरी
                  </CardTitle>
                </CardHeader>
                <CardContent>
                      ...
                      <div key={i} className="flex items-start gap-3 bg-red-50 rounded-xl p-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="font-semibold text-red-800">{gap.label}</span>
                            <Badge className="bg-red-100 text-red-700 text-xs">
                              {gap.status === 'missing' ? 'NOT COVERED' : 'LOW COVERAGE'}
                            </Badge>
                          </div>
                          <p className="text-sm text-red-700 hindi-text">{gap.labelHindi}</p>
                          <p className="text-sm text-gray-600 mt-1">{gap.reason}</p>
                          <p className="text-xs text-orange-600 hindi-text mt-0.5">{gap.reasonHindi}</p>
                          {gap.status === 'low' && (
                            <p className="text-sm text-red-600 mt-1 font-medium">
                              Current: ₹{(gap.currentCoverage / 100000).toFixed(1)}L →
                              Need: ₹{(gap.recommendedCoverage / 100000).toFixed(1)}L
            ...
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
              {[
                { key: 'overview', label: 'Coverage Overview', hindi: 'कवरेज अवलोकन' },
                { key: 'details', label: 'Detailed Analysis', hindi: 'विस्तृत विश्लेषण' },
                { key: 'recommendations', label: 'Recommendations', hindi: 'सिफारिशें' },
              ].map(tab => (
                <button
                  key={tab.key}
                ...
                >
                  <div>{tab.label}</div>
                  <div className="text-xs hindi-text opacity-70">{tab.hindi}</div>
                </button>
              ))}
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {result.gaps.map((gap, i) => (
                  <Card key={i} className={`border-0 shadow-sm ${
                    gap.status === 'good' ? 'bg-green-50' :
                    gap.status === 'low' ? 'bg-yellow-50' :
                    'bg-red-50'
                  }`}>
                    <CardContent className="p-5">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-blue-600 hindi-text">{gap.labelHindi}</p>
                          </div>
                        </div>
                        {gap.status === 'good'
                      ...
                      <div className="space-y-2">
                        ...
                        <Progress
                          ...
                          className={`h-2 ${
                            gap.status === 'good' ? '[&>div]:bg-green-500' :
                            gap.status === 'low' ? '[&>div]:bg-yellow-500' :
                            '[&>div]:bg-red-500'
                          }`}
                        />
                        <div className="flex justify-between text-xs">
                          <span className="text-gray-500">
                            Have: {gap.currentCoverage > 0
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="font-semibold text-gray-900">{gap.label}</h3>
                            <span className="text-sm hindi-text text-blue-600">{gap.labelHindi}</span>
                            <Badge className={
                              gap.status === 'good' ? 'bg-green-100 text-green-700' :
                              gap.status === 'low' ? 'bg-yellow-100 text-yellow-700' :
                              'bg-red-100 text-red-700'
                            }>
                              {gap.status === 'good' ? 'Well Covered ✓' :
                               gap.status === 'low' ? 'Low Coverage ⚠️' :
                               'Not Covered ✗'}
                            </Badge>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-3 mb-3">
                            <div className="bg-gray-50 rounded-lg p-2 text-center">
                              <p className="text-xs text-gray-500">Your Coverage</p>
                            ...
                            <div className={`rounded-lg p-2 text-center ${
                              ...
                              <p className="text-xs text-gray-500">Gap Amount</p>
                              <p className={`font-bold ${gap.gap === 0 ? 'text-green-700' : 'text-red-700'}`}>
                                {gap.gap === 0 ? 'None!' : `₹${(gap.gap/100000).toFixed(1)}L`}
                              </p>
                            </div>
                          </div>
                          ...
                          {gap.policies.length > 0 && (
                            <div className="mt-2">
                              <p className="text-xs text-gray-500 mb-1">Your policies in this category:</p>
                      ...
                      {[
                        {
                          ...
                          hindi: 'ऑनलाइन टर्म इंश्योरेंस खरीदें — ऑफलाइन से 30-40% सस्ता'
                        },
                        {
                          icon: '📅',
                          tip: 'Buy health insurance young — premiums are lower and fewer exclusions',
                          hindi: 'युवावस्था में स्वास्थ्य बीमा लें — प्रीमियम कम और अपवाद कम'
                        },
                        {
                          icon: '👨‍👩‍👧‍👦',
                          tip: 'Family floater health plan covers entire family at lower cost',
                          hindi: 'फैमिली फ्लोटर प्लान पूरे परिवार को कम लागत पर कवर करता है'
                        },
                        {
                          icon: '📈',
                          tip: 'Review and increase sum insured every 2-3 years for inflation',
                          hindi: 'महंगाई के लिए हर 2-3 साल में बीमा राशि बढ़ाएं'
                        },
                        {
                          icon: '🔄',
                          tip: 'Port your health insurance to a better insurer without losing benefits',
                          hindi: 'लाभ खोए बिना बेहतर बीमाकर्ता के पास स्वास्थ्य बीमा पोर्ट करें'
                        },
                        {
                          icon: '📋',
                          tip: 'Always disclose pre-existing conditions to avoid claim rejection',
                          hindi: 'क्लेम अस्वीकृति से बचने के लिए हमेशा पहले से मौजूद बीमारी बताएं'
                        },
                      ].map((item, i) => (
                        <div key={i} className="flex gap-3 p-3 bg-blue-50 rounded-xl">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-blue-600 hindi-text mt-0.5">{item.hindi}</p>
                          </div>
                        </div>
                      ))}
                    </div>
